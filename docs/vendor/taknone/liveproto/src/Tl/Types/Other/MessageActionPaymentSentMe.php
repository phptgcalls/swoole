<?php

declare(strict_types = 1);

namespace Tak\Liveproto\Tl\Types\Other;

use Tak\Liveproto\Utils\Binary;

use Tak\Liveproto\Utils\Instance;

/**
 * @param string currency long total_amount bytes payload paymentcharge charge true recurring_init true recurring_used paymentrequestedinfo info string shipping_option_id int subscription_until_date
 * @return MessageAction
 */

final class MessageActionPaymentSentMe extends Instance {
	public function request(string $currency,int $total_amount,string $payload,object $charge,? true $recurring_init = null,? true $recurring_used = null,? object $info = null,? string $shipping_option_id = null,? int $subscription_until_date = null) : Binary {
		$writer = new Binary(true);
		$writer->writeInt(0xffa00ccc);
		$flags = 0;
		$flags |= is_null($recurring_init) ? 0 : (1 << 2);
		$flags |= is_null($recurring_used) ? 0 : (1 << 3);
		$flags |= is_null($info) ? 0 : (1 << 0);
		$flags |= is_null($shipping_option_id) ? 0 : (1 << 1);
		$flags |= is_null($subscription_until_date) ? 0 : (1 << 4);
		$writer->writeInt($flags);
		$writer->tgwriteBytes($currency);
		$writer->writeLong($total_amount);
		$writer->tgwriteBytes($payload);
		if(is_null($info) === false):
			$writer->write($info->read());
		endif;
		if(is_null($shipping_option_id) === false):
			$writer->tgwriteBytes($shipping_option_id);
		endif;
		$writer->write($charge->read());
		if(is_null($subscription_until_date) === false):
			$writer->writeInt($subscription_until_date);
		endif;
		return $writer;
	}
	public function response(Binary $reader) : object {
		$result = array();
		$flags = $reader->readInt();
		if($flags & (1 << 2)):
			$result['recurring_init'] = true;
		else:
			$result['recurring_init'] = false;
		endif;
		if($flags & (1 << 3)):
			$result['recurring_used'] = true;
		else:
			$result['recurring_used'] = false;
		endif;
		$result['currency'] = $reader->tgreadBytes();
		$result['total_amount'] = $reader->readLong();
		$result['payload'] = $reader->tgreadBytes();
		if($flags & (1 << 0)):
			$result['info'] = $reader->tgreadObject();
		else:
			$result['info'] = null;
		endif;
		if($flags & (1 << 1)):
			$result['shipping_option_id'] = $reader->tgreadBytes();
		else:
			$result['shipping_option_id'] = null;
		endif;
		$result['charge'] = $reader->tgreadObject();
		if($flags & (1 << 4)):
			$result['subscription_until_date'] = $reader->readInt();
		else:
			$result['subscription_until_date'] = null;
		endif;
		return new self($result);
	}
}

?>